...
The user option ~codex-full-auto~ enables fully autonomous mode by passing Codex's current ~--dangerously-bypass-approvals-and-sandbox~ flag.  The default is ~nil~.  When non-nil, this overrides both ~codex-sandbox-mode~ and ~codex-approval-policy~.  The option name is retained for compatibility with older =codex.el= configurations; use with caution.
...
The user option ~codex-enable-hooks~ controls whether =codex.el= automatically configures the Codex CLI hooks system.  The default is ~t~.  When enabled, activating ~codex-mode~ ensures that ~config.toml~ has ~hooks = true~ under ~[features]~ and that ~hooks.json~ contains entries for the hook types that =codex.el= handles.

The user option ~codex-hooks-config-path~ is the path to the Codex ~config.toml~ file.  The default is ~"config.toml"~ under ~CODEX_HOME~, or under ~"~/.codex"~ when ~CODEX_HOME~ is unset.

The user option ~codex-hooks-json-path~ is the path to the Codex ~hooks.json~ file.  The default is ~"hooks.json"~ under ~CODEX_HOME~, or under ~"~/.codex"~ when ~CODEX_HOME~ is unset.
...
2. Ensures that ~hooks.json~ has entries for the hook types handled by =codex.el=: ~Stop~, ~SessionStart~, ~PreToolUse~, ~PermissionRequest~, ~PostToolUse~, ~PreCompact~, ~PostCompact~, and ~UserPromptSubmit~.
...
Current Codex releases may require reviewing and trusting command hooks from the ~/hooks~ slash command before they run. =codex.el= installs and repairs the hook definitions, but it does not bypass Codex's hook trust review or manage inline ~[hooks]~ tables, plugin hooks, or hook state in ~config.toml~.
