...
Some CLI tools use near-white background colors for card-like UI elements (code blocks, tool outputs, etc.).  On dark Emacs themes, these render as bright rectangles.  The background color remapping feature detects these light backgrounds and strips them after each batch of terminal output.

The user option ~codex-remap-light-backgrounds~ enables or disables this feature.  The default is ~t~.  When non-nil, background colors whose perceived luminance exceeds ~codex-light-background-threshold~ are stripped (or replaced; see below).

The user option ~codex-card-background~ specifies the replacement color.  When ~nil~ (the default), light backgrounds are stripped entirely so the terminal's default background shows through.  Set this to a color string (e.g. ~"#1a1b2e"~) to use a fixed replacement color instead.
...
The command ~codex-resume~ resumes a previous Codex session by running ~codex resume~.  You are always prompted for an instance name.  With a prefix argument, it passes ~--last~ to resume the most recent session.

The command ~codex-fork~ forks a previous session by running ~codex fork~.  Like ~codex-resume~, it prompts for an instance name and accepts a prefix argument for ~--last~.
