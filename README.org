...
The user option ~codex-enable-prompt-autosuggestions~ controls whether =codex.el= styles prompt autosuggestions in =eat= buffers.  The default is ~t~.  Codex renders placeholder and suggestion text after the terminal cursor; when that text arrives in Emacs without the CLI's dim styling, =codex.el= recognizes known suggestions and applies ~codex-prompt-autosuggestion-face~.  The options ~codex-prompt-autosuggestion-placeholders~, ~codex-prompt-autosuggestion-history-path~, and ~codex-prompt-autosuggestion-history-limit~ control which built-in placeholder strings and recent history entries are recognized as autosuggestions.  In =eat= Codex buffers, =TAB= first accepts the visible autosuggestion when there is one; otherwise it is sent through to the Codex TUI as a normal tab.
