...
The user option ~codex-eat-preserve-scrollback~ controls whether Codex =eat= buffers strip CSI erase-display commands before Eat processes output.  The default is ~t~.  Codex runs with ~--no-alt-screen~ by default so the Emacs buffer can serve as session history, but TUI redraws can still emit erase-display sequences that make Eat delete retained buffer text.  Leave this enabled unless you explicitly prefer terminal-accurate clear-screen behavior over preserved scrollback.
