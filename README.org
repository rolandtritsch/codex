...
Codex is spawned with ~COLORTERM~ unset, which causes the CLI to downgrade from 24-bit RGB escape codes to the 256-color indexed palette.  The indexed colors route through eat's ~eat-term-color-*~ faces, which the Emacs theme already controls.  As a result Codex's palette follows whatever Emacs theme is active (light or dark), without any post-hoc text-property manipulation on the Codex buffer.  No user options are exposed for this behavior.
